...
const projectItems = [
  ...
  {
    id: "project-4",
    title: "Invoicing SaaS App",
    description:
      "Automated invoicing platform with recurring billing, payment reminders, and detailed financial reporting.",
    link: "https://example.com/invoicing",
  }, */
];
...
const Demos = () => {
  ...
  const [activeSectionIndex, setActiveSectionIndex] = useState(() => {
    ...
    return Math.max(
      0,
      SECTIONS.findIndex((s) => s.id === initial),
    );
  });
  ...
  useEffect(() => {
    const initialIdx = Math.max(
      0,
      SECTIONS.findIndex(
        (s) => s.id === (searchParams.get("section") || "tech-stack"),
      ),
    );
  ...
  const TechStackLogo = ({
    ...
    return (
      <div
        ref={(el) => {
          if (el) allTechLogosRef.current[index] = el;
        }}
