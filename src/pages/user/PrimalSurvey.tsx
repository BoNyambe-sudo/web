...
const PrimalSurvey = () => {
  ...
  const [sliderValue, setSliderValue] = useState(() => {
    ...
    if (stored) {
      try {
        ...
        return parsed.responses?.missing_revenue || 25000;
      } catch {
        return 25000;
      }
    }
  ...
  const [tempWebsiteUrl, setTempWebsiteUrl] = useState(() => {
    ...
    if (stored) {
      try {
        const parsed = JSON.parse(stored);
        return parsed.responses?.website_url || "";
      } catch {
        return "";
      }
    }
  ...
  useEffect(() => {
    if (!mountedRef.current || !questionRef.current) return;
  ...
  const handleNext = () => {
    ...
    nextStep();
    setTimeout(() => skipRef.current?.focus(), 50);
  };
  ...
  const handleAnswer = (key: ResponseKey, value: string | number) => {
  ...
  const getOutcomeDetails = () => {
    ...
    const goal = responses.business_goal;
